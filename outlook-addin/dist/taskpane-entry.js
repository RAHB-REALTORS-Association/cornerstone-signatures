(() => {
  ...
  var CryptoOps = class {
    ...
    async signTokenBindingJwt(header, payload, kid, correlationId) {
      ...
      try {
        ...
        signTokenBindingJwtMeasurement == null ? void 0 : signTokenBindingJwtMeasurement.end({
          success: true,
          ...telemetry
        });
  ...
  async function getAccessToken({ interactive = false, scopes = GRAPH_TOKEN_SCOPES } = {}) {
  ...
  async function getManagedPreferences({ interactive = false } = {}) {
    const accessToken = await getSignatureApiAccessToken({ interactive });
  ...
  // outlook-addin/src/taskpane-entry.js
  ...
  async function connect() {
    ...
    try {
      ...
      const organizationName = (_a = managedSignature.branding) == null ? void 0 : _a.organizationName;
      if (organizationName) {
      ...
      const userName = typeof managedSignature.user === "string" ? managedSignature.user : ((_b = managedSignature.user) == null ? void 0 : _b.displayName) || ((_c = managedSignature.user) == null ? void 0 : _c.email) || "";
      const details = [managedSignature.templateName, managedSignature.version && `version ${managedSignature.version}`].filter(Boolean).join(", ");
      ...
      const message = error instanceof ManagedSignatureError ? "Connected to Microsoft 365, but Cornerstone Signatures could not load your managed signature. Contact IT if this continues." : "Could not refresh the signature in this message. Check your account and try again.";
